...
```
获取下一个我们应该布局视图元素。 __接下来涉及到缓存，可以先看看缓存.md__
...
第一步 nextViewFromScrapList()从ScrapList查找
```java
...
```
第二步 recycler.getViewForPosition(mCurrentPosition) 最后会调用tryGetViewHolderForPositionByDeadline(position, dryRun, FOREVER_NS)
```java
...
试图通过Recycler废料，缓存，RecycledViewPool或直接创建ViewHolder来获取给定位置的ViewHolder。
...
...
    ...
    // 尝试在mChangedScrap查找 分别通过 position 和 id 查找
...
```
#### 1)
这块代码逻辑比较简单。可以自己再翻看源码过一遍。
先调用``getScrapOrHiddenOrCachedHolderForPosition(position, dryRun)``方法获取 __holder__ ,分别遍历 ``mAttachedScrap``,``mHiddenViews``,``mCachedViews``，直到找到符合条件的 __ViewHolder__ 返回，否则返回 ``null``。
接下来如果找到 __holder__ 会再验证是不是当前position。如果无效，则从``mAttachedScrap``中移除，并加入到``mCacheViews``或者Pool中，并且将holder至null，走下一级缓存判断。
```java
ViewHolder tryGetViewHolderForPositionByDeadline(int position,
        boolean dryRun, long deadlineNs) { 
    ...
...
#### 2)
todo mAdapter.hasStableIds() 这个没用过不知道怎么理解。先往下看吧
``getScrapOrCachedViewForId(mAdapter.getItemId(type, dryRun)`` 这个方法跟 1) 逻辑基本相似。在 ``mAttachedScrap`` 和 ``mCachedViews``分别查找 holder ,只是这回判断多了一层 viewType。
```java
ViewHolder tryGetViewHolderForPositionByDeadline(int position,
        boolean dryRun, long deadlineNs) {
    if (holder == null) {
...
接下来会通过``mViewCacheExtension``,``mRecyclerPool``查找holder，如果都没有就会调用``mAdapter.createViewHolder(RecyclerView.this, type)``。
[TOC] 
